...
This wrapper generates outputs with the current modern `--patch-docx` path for the five positive modern cases, writes `out/modern-docx-omml-generated/modern_docx_omml_generated_outputs.json`, checks basic DOCX package openability, then validates that manifest structurally.
...
This 5-case generated-output gate is the official precondition before product behavior changes. It reports the openability summary, structural summary, writes the JSON gate artifact above, and exits nonzero if openability or structural validation fails.

Modern-path PR/release checklist (operational):

- run locally before merge: `mvn -q -Dtest=DocxMathPatchWorkflowTest,DocxToHtmlCliPatchSummaryJsonlTest test`
- run locally before merge: `python3 scripts/workflow/run_modern_docx_omml_smoke.py`
- run locally before merge: `python3 scripts/workflow/run_modern_docx_omml_generated_output_gate.py`
- required blocking check: `modern-docx-omml-generated-output-gate` (must be green)
- first place to inspect on failure: uploaded artifact `modern-docx-omml-generated-output-gate-report` and the GitHub job summary emitted by `scripts/workflow/render_modern_docx_omml_gate_summary.py`
